...
public class FSharpSensor implements Sensor {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(FSharpSensor.class);
  ...
  private void analyze() {
    ...
    appendLine(sb, "  </Settings>");
    appendLine(sb, "  <Rules>");
    for (ActiveRule activeRule : ruleProfile.getActiveRulesByRepository(FSharpPlugin.REPOSITORY_KEY)) {
      appendLine(sb, "    <Rule>");
    ...
    private void handleIssueTag(Issuable issuable) throws XMLStreamException {
      ...
      while (stream.hasNext()) {
        ...
        if (next == XMLStreamConstants.END_ELEMENT && "Issue".equals(stream.getLocalName())) {
          ...
          builder.ruleKey(RuleKey.of(FSharpPlugin.REPOSITORY_KEY, id));
